...
{//初始化哈夫曼数，要求用户输入字符和相应权值
    ...
    char x;
    FILE *fp;
    fp = fopen("/Users/liyuteng/Desktop/hfmtree.txt", "rt");//对文件hfmtree以读文本的形式打开
    if (fp == NULL) {
        Hfm = InputHuffman(Hfm);//调用InputHuffman函数，用户输入字符和相应权值存入哈夫曼数中
        fp = fopen("/Users/liyuteng/Desktop/hfmtree.txt", "wt");
        fprintf(fp, "%d\n", Hfm.length);
        ...
        if (x == 'Y'|| x == 'y') {
            ...
            fp = fopen("/Users/liyuteng/Desktop/hfmtree.txt", "w+");
            fprintf(fp, "%d\n", Hfm.length);
            for (i = 1; i <= Hfm.length; i++)
                fprintf(fp, "%c %d  ", Hfm.c[i], Hfm.HT[i].weight);
            rewind(fp);
...
{//利用已建好的Huffman树（如不在内存，则从文件hfmtree中读入），
    ...
    do {
        ...
        printf("\n");
        if (k == 1) {
            if ((fw = fopen("/Users/liyuteng/Desktop/Tobetrans.txt", "r+")) == NULL) {
                ...
                fscanf(fw, "%s", ch);
                fclose(fw);
                fp = fopen("/Users/liyuteng/Desktop/codefile.txt", "wt+");
                k = 0;
            }
        } else if (k == 2) {
            printf("请输入要编码的字符串: ");
...
{//利用已建好的Huffman树将文件codefile中的代码进行译码，结果存入文件TextFile中。
    ...
    n = Hfm.length;
    printf("\n--译码--\n");
    if((fp = fopen("/Users/liyuteng/Desktop/codefile.txt", "r+")) == NULL) {
        printf("请输入哈夫曼编码:");
        scanf("%s",d);
    } else {
    ...
    fp = fopen("/Users/liyuteng/Desktop/Textfile.txt","wt+");//以写入文件的形式打开TextFile
    while (d[j]) {
        p = &Hfm.HT[2*n-1];
...
{//将文件codefile以紧凑格式显示在终端上，每行50 个代码。
    ...
    fprint = fopen("/Users/liyuteng/Desktop/codefile.txt", "rb");
    while (!feof(fprint)) {
        ch = fgetc(fprint);
        printf("%c", ch);
        m++;
